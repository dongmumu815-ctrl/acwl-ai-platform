...
"""
        ...
        try:
            ...
            sftp = ssh.open_sftp()
            with open("temp_harbor_v2.yml", "w", newline='\n') as f:
                f.write(harbor_yml)
            sftp.put("temp_harbor_v2.yml", "/tmp/harbor.yml")
            sftp.close()
            ...
            sftp = ssh.open_sftp()
            with open("temp_official_compose_v2.yml", "w", newline='\n') as f:
                f.write(compose)
            sftp.put("temp_official_compose_v2.yml", "/tmp/docker-compose.yml")
            sftp.close()
            ...
            print("Official Reinstall v2 Complete.")
            
        except Exception as e:
            print(f"Error: {e}")
        finally:
            ssh.close()
            if os.path.exists("temp_harbor_v2.yml"): os.remove("temp_harbor_v2.yml")
            if os.path.exists("temp_official_compose_v2.yml"): os.remove("temp_official_compose_v2.yml")
