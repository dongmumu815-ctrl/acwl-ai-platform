...
"""
        ...
        try:
            ...
            sftp = ssh.open_sftp()
            with open("temp_harbor_v5.yml", "w", newline='\n') as f:
                f.write(harbor_yml)
            sftp.put("temp_harbor_v5.yml", "/tmp/harbor.yml")
            sftp.close()
            
            ssh.exec_command(f"echo '{PASS}' | sudo -S mv /tmp/harbor.yml /data/harbor/harbor.yml")
            
            # Run Prepare (Mount /config)
            print("Running Prepare Container (Correct Mount)...")
            # We mount /data/harbor/common/config to /config
            # And /data/harbor to /data (for secret keys persistence? or secret key is in config?)
            # Secret keys are usually in /data/secret/keys if configured?
            # Let's check where prepare writes secrets. Output said: /data/secret/keys/secretkey
            # So we need to mount /data/harbor to /data as well.
            ...
            # Ignore Errno 2 about docker-compose.yml missing, as we supply it manually
            ...
            if "no such file" in out or "env" not in out:
            ...
            sftp = ssh.open_sftp()
            with open("temp_official_compose_v5.yml", "w", newline='\n') as f:
                f.write(compose)
            sftp.put("temp_official_compose_v5.yml", "/tmp/docker-compose.yml")
            sftp.close()
            ...
            print("Official Reinstall v5 Complete.")
            
        except Exception as e:
            print(f"Error: {e}")
        finally:
            ssh.close()
            if os.path.exists("temp_harbor_v5.yml"): os.remove("temp_harbor_v5.yml")
            if os.path.exists("temp_official_compose_v5.yml"): os.remove("temp_official_compose_v5.yml")
