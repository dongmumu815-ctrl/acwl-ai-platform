...
async def main():
    async with AsyncSessionLocal() as session:
        ...
        # Fix Bug: EXT_ENDPOINT is not defined in shell script, use EXT_URL instead
        script = script.replace('realm: $EXT_ENDPOINT/service/token', 'realm: $EXT_URL/service/token')
        
        # Fix Private Key Generation in Script (Force PKCS#1 with -traditional for OpenSSL 3.0)
        ...
        # We know target is OpenSSL 3.0, so use -traditional
        ...
        try:
            ...
            # Copy docker-compose to /data/harbor for convenience
