...
def _generate_markdown_doc(api: CustomApi, api_fields: list = None) -> str:
    ...
    doc.append("| 参数名 | 类型 | 必填 | 描述 |")
    doc.append("|--------|------|------|------|")
    doc.append("| since | String | 否 | 最后同步时间，ISO8601格式（如：2024-01-01T12:00:00Z） |")
    doc.append("| include_unchanged | Boolean | 否 | 是否包含未变更的数据，默认false |")
    doc.append("| format | String | 否 | 返回格式：json（默认）或 xml |")
    doc.append("")
    
    doc.append("#### since 参数说明")
    doc.append("")
    doc.append("- **用途**: 增量同步，只返回指定时间之后有变更的数据")
    doc.append("- **格式**: ISO8601标准时间格式，支持时区")
    doc.append("- **示例**: `2024-01-01T12:00:00Z` 或 `2024-01-01T12:00:00+08:00`")
    doc.append("- **行为**: 当提供此参数时，系统只返回在此时间之后有更新的结果数据")
    doc.append("- **配合回调**: 建议结合回调通知机制，在收到变更通知后使用此参数重新同步")
    doc.append("")
    ...
    doc.append("```bash")
    doc.append("# 获取完整结果")
    doc.append(f"GET /api/v1/results/{api.api_code}/batch_123")
    doc.append("")
    doc.append("# 增量同步（只获取2024-01-01 12:00:00之后的变更）")
    doc.append(f"GET /api/v1/results/{api.api_code}/batch_123?since=2024-01-01T12:00:00Z")
    doc.append("")
    doc.append("# 包含未变更数据的增量同步")
    doc.append(f"GET /api/v1/results/{api.api_code}/batch_123?since=2024-01-01T12:00:00Z&include_unchanged=true")
    doc.append("```")
    doc.append("")
    ...
    doc.append("")
    doc.append("### 接口信息")
    doc.append(f"- **接口地址**: `/api/v1/results/{api.api_code}/{{batch_id}}/{{status}}/{{datetime}}`")
    doc.append("- **请求方法**: `GET`")
    ...
    doc.append("| last_modified | String | 数据最后修改时间（ISO8601格式） |")
    doc.append("| created_at | String | 批次创建时间（ISO8601格式） |")
    doc.append("| completed_at | String | 批次完成时间（ISO8601格式），仅在completed/failed时返回 |")
    doc.append("| has_changes | Boolean | 自since时间以来是否有变更（仅在提供since参数时返回） |")
    doc.append("")
    ...
    doc.append('        "last_modified": "2024-01-01T10:30:00Z",')
    doc.append('        "created_at": "2024-01-01T10:00:00Z",')
    ...
    doc.append('        "last_modified": "2024-01-01T12:00:00Z",')
    doc.append('        "created_at": "2024-01-01T10:00:00Z",')
    doc.append('        "completed_at": "2024-01-01T12:00:00Z"')
    doc.append('    }')
    doc.append("}")
    ...
    doc.append('        "iv": null,')
    doc.append('        "result_sign": null,')
    doc.append('        "last_modified": "2024-01-01T11:00:00Z",')
    doc.append('        "created_at": "2024-01-01T10:00:00Z",')
    doc.append('        "completed_at": "2024-01-01T11:00:00Z",')
