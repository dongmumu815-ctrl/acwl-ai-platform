...
class Settings(BaseSettings):
    ...
    AGENT_SKILL_API_KEY: str = Field(default="change-me-in-production", description="Agent技能直调API Key")
    ...
    DB_PASSWORD: str = Field(default="change-me", description="数据库密码")
    DB_NAME: str = Field(default="acwl-ai-data", description="数据库名称")
    DB_CHARSET: str = Field(default="utf8mb4", description="数据库字符集")
    
    # Doris配置（用于数据上传日志展示）
    DORIS_HOST: str = Field(default=os.getenv("DORIS_HOST", "localhost"), description="Doris主机")
    ...
    DORIS_PASSWORD: str = Field(default=os.getenv("DORIS_PWD", "change-me"), description="Doris密码")
    DORIS_DATABASE: str = Field(default=os.getenv("DORIS_DB", "analytics-logs"), description="Doris数据库")
    ...
    LOG_FILE: str = Field(default="logs/acwl-platform.log", description="日志文件")
    ...
    MINIO_ACCESS_KEY: str = Field(default="your-access-key", description="MinIO访问密钥")
    MINIO_SECRET_KEY: str = Field(default="your-secret-key", description="MinIO秘密密钥")
    MINIO_SECURE: bool = Field(default=False, description="是否使用HTTPS")
    MINIO_BUCKET_NAME: str = Field(default="ai-platform-data", description="存储桶名称")
