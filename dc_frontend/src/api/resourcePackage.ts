...
export const resourcePackageApi = {
   ...
   */
  create(
    data: ResourcePackageCreateRequest,
  ): Promise<ApiResponse<ResourcePackage>> {
    return post("/resource-packages/", data, {
      permission: "data:resource_package:create",
    });
  },
  ...
  get(id: number): Promise<ApiResponse<ResourcePackage>> {
    return get(`/resource-packages/${id}`, undefined, {
      permission: "data:resource_package:view",
    });
  },
  ...
  update(
    id: number,
    data: ResourcePackageUpdateRequest,
  ): Promise<ApiResponse<ResourcePackage>> {
    return put(`/resource-packages/${id}`, data, {
      permission: "data:resource_package:edit",
    });
  },
  ...
  delete(id: number): Promise<ApiResponse<{ message: string }>> {
    return del(`/resource-packages/${id}`, {
      permission: "data:resource_package:delete",
    });
  },
   ...
   */
  search(
    params: ResourcePackageSearchRequest,
  ): Promise<ApiResponse<ResourcePackageListResponse>> {
    return post("/resource-packages/search", params, {
      permission: "data:resource_package:search",
    });
  },
  ...
  query(
    id: number,
    params: ResourcePackageQueryRequest,
  ): Promise<ApiResponse<ResourcePackageQueryResponse>> {
    return post(`/resource-packages/${id}/query`, params, {
      permission: "data:resource_package:query",
    });
  },
  ...
  exportAllResults(
    ...
    return post(`/resource-packages/${id}/export-all`, payload, {
      permission: "data:resource_package:export",
    });
  },
  ...
  getLatestExportFile(
    id: number,
  ): Promise<ApiResponse<{ download_url: string; file_id: number }>> {
    return get(`/resource-packages/${id}/export-file`, undefined, {
      permission: "data:resource_package:export:file",
    });
  },
   ...
   */
  getLatestExportFileByQuery(
    id: number,
    payload: Record<string, any>,
  ): Promise<ApiResponse<{ download_url: string; file_id: number }>> {
    return post(`/resource-packages/${id}/export-file/check`, payload, {
      permission: "data:resource_package:export:file",
    });
  },
  ...
  deleteExportFile(
    ...
    return del(`/resource-packages/${id}/export-file/${fileId}`, {
      permission: "data:resource_package:export:file:delete",
    });
  },
  ...
  generateExcel(
    ...
    return post(`/resource-packages/${id}/generate-excel`, payload, {
      timeout: 180000,
      permission: "data:resource_package:excel",
    });
  },
  ...
  downloadLatest(id: number): Promise<ApiResponse<any>> {
    return post(
      `/resource-packages/${id}/download-latest`,
      {},
      { permission: "data:resource_package:download" },
    );
  },
   ...
   * 获取安全查询历史记录
   */
  getSecureQueryHistory(
    id: number,
    page: number = 1,
    size: number = 20,
  ): Promise<ApiResponse<any>> {
    return get(
      `/resource-packages/${id}/query-history?page=${page}&size=${size}`,
      undefined,
  ...
  > {
    return get(
      `/resource-packages/${id}/files`,
      { page, size },
      { permission: "data:resource_package:files:view" },
    );
  },
  ...
  downloadFile(
    ...
    ApiResponse<{ download_url: string; filename: string; object_path: string }>
  > {
    return post(
      `/resource-packages/${id}/files/${fileId}/download`,
      {},
      { permission: "data:resource_package:download" },
    );
  },
...
export const templateApi = {
  ...
  list(params?: {
    ...
    if (params?.data_resource_id)
      query.data_resource_id = params.data_resource_id;
    if (params?.indices && params.indices.length > 0)
      query.indices = params.indices.join(",");
  ...
  get(id: number, type: PackageType): Promise<{ data: Template }> {
    if (type === PackageType.ELASTICSEARCH) {
      return get(`/es/templates/${id}`, undefined, {
        permission: "data:es:templates:view",
      });
    }
    return get(`/sql/templates/${id}`, undefined, {
      permission: "data:sql:templates:view",
    });
  },
