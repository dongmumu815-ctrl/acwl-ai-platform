...
export const datasourceApi = {
  ...
  getDatasource(datasourceId: number) {
    return get<Datasource>(`/datasources/${datasourceId}`, undefined, {
      permission: "data:datasource:view",
    });
  },
  ...
  updateDatasource(datasourceId: number, data: Partial<DatasourceForm>) {
    const { database, ...rest } = data;
    const backendData =
      database !== undefined ? { ...rest, database_name: database } : rest;
  ...
  testDatasourceConnection(
    ...
    return post<{
      ...
      connection_info?: any;
      error_details?: string;
    }>(`/datasources/${datasourceId}/test`, params, {
      permission: "data:datasource:test",
    }).then((res) => res.data);
  },
  ...
  /** 获取数据源统计信息 */
  getDatasourceStats() {
    return get<DatasourceStats>("/datasources/stats/overview", undefined, {
  ...
  getDatasourceTestLogs(
    ...
    return get<PaginatedResponse<any>>(
      `/datasources/${datasourceId}/test-logs`,
      params,
      { permission: "data:datasource:logs:view" },
    );
  },
  ...
  getDatasourceTemplates(datasourceType?: DatasourceType) {
    ...
    return get<Record<string, any>>("/datasources/templates/", params, {
      permission: "data:datasource:templates:view",
    }).then((res) => res.data?.data ?? res.data);
  },
  ...
  getDatasourceConnectionInfo(datasourceId: number) {
    return get<Record<string, any>>(
      `/datasources/${datasourceId}/connection-info`,
      undefined,
      { permission: "data:datasource:connection:view" },
    ).then((res) => res.data?.data ?? res.data);
  },

  /** 执行数据源查询 */
  executeDatasourceQuery(datasourceId: number, data: DatasourceQuery) {
    return post<{ columns: string[]; rows: any[][]; total: number }>(
      `/datasources/${datasourceId}/query`,
      data,
      { permission: "data:datasource:query" },
    ).then((res) => res.data?.data ?? res.data);
  },
};
...
export const testTempDatasourceConnection =
  datasourceApi.testTempDatasourceConnection;
...
export const getDatasourceConnectionInfo =
  datasourceApi.getDatasourceConnectionInfo;
...
export const DATASOURCE_TYPES: DatasourceTypeOption[] = [
  ...
  {
    value: DatasourceType.ELASTICSEARCH,
    label: "Elasticsearch",
    icon: "elasticsearch",
  },
];
...
export function getDatasourceStatusConfig(
  status: DatasourceStatus,
): DatasourceStatusOption {
...
export function getConnectionParamsTemplate(
  type: DatasourceType,
): Record<string, any> {
...
export function getPoolConfigTemplate(
  type: DatasourceType,
): Record<string, any> {
  return {
    pool_size: 5,
