...
export const DATA_INSIGHT_CONFIG = {
  ...
  endpoints: {
    ...
    explorer: {
      ...
      getSQLHistory: (datasourceId: number) => `/explorer/sql-history/${datasourceId}`,
      getSQLHistoryDetail: (historyId: number) => `/explorer/sql-history-detail/${historyId}`,
      deleteSQLHistory: (historyId: number) => `/explorer/sql-history/${historyId}`,
      ...
      // 表变更日志
      getTableChangeLog: (datasourceId: number) => `/explorer/table-change-log/${datasourceId}`
    }
  }
}
