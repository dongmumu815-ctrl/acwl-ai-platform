...
export const useUserStore = defineStore("user", () => {
  ...
  const restoreFromStorage = (): void => {
    try {
      ...
      if (storedToken && storedUser) {
        ...
        usedPermissions.value = storedUsedPerms
          ? JSON.parse(storedUsedPerms)
          : [];
      }
    } catch (error) {
  ...
  const hasAnyPermission = (permissionList: string[]): boolean => {
    ...
    if (isAdmin.value) return true;
    return permissionList.some((required) =>
      permissions.value.some((p) => permissionMatches(p, required)),
    );
  };
  ...
  const hasAllPermissions = (permissionList: string[]): boolean => {
    if (!permissionList || permissionList.length === 0) return true;
    if (isAdmin.value) return true;
    return permissionList.every((required) =>
      permissions.value.some((p) => permissionMatches(p, required)),
    );
  };
  ...
  const refreshPermissions = async (): Promise<void> => {
    try {
      ...
      if (resp.success) {
        ...
        roles.value = Array.isArray(data?.role_codes)
          ? data.role_codes
          : roles.value;
  ...
  const recordUsedPermission = (perm: string | string[]): void => {
    ...
    localStorage.setItem(
      "used_permissions",
      JSON.stringify(usedPermissions.value),
    );
  };
