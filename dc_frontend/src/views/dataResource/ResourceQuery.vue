<template>
  <div class="resource-query-container">
    ...
    <div class="query-type-section">
      <el-card class="type-card">
        ...
        >
          ...
          <el-radio-button label="elasticsearch"
            >Elasticsearch查询</el-radio-button
          >
        </el-radio-group>
      </el-card>
    </div>
    ...
    <!-- SQL查询组件 -->
    <div v-if="queryType === 'sql'" class="sql-query-section">
    ...
    <!-- ES查询组件 -->
    <div v-if="queryType === 'elasticsearch'" class="es-query-section">
      <ESQueryBuilder
...
const currentResourceId = computed(() => {
  ...
  const resource = sqlResources.value.find((r) => {
    ...
    console.log(
      `🔍 匹配检查: resource.datasource_id(${r.datasource_id}) === initialDatasourceId(${initialDatasourceId.value}) = ${match}`,
    );
    return match;
  });
...
const dataSecurity = computed(() => {
  return {
    enableFieldMasking:
      userStore.user?.dataSecurity?.enableFieldMasking || false,
    enableRowLevelSecurity:
      userStore.user?.dataSecurity?.enableRowLevelSecurity || false,
...
const onSQLQueryExecute = async (queryData) => {
  ...
  try {
    ...
    const row_count =
      apiResult?.row_count ?? (Array.isArray(data) ? data.length : 0);
    ...
    if (success) {
      ...
      if (columns && data) {
        ...
        console.log(
          `✅ 查询执行成功，返回 ${row_count} 条记录，执行时间: ${execution_time}ms`,
        );
...
const onESQuerySave = async (queryData) => {
  try {
    ...
    const templateData = {
      ...
      tags: queryData.tags || [],
      isTemplate:
        queryData.isTemplate !== undefined ? queryData.isTemplate : true,
    };
