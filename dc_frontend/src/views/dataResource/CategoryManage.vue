<template>
  <div class="category-manage-container">
            ...
            <template #default="{ node, data }">
              <div class="tree-node">
                <div class="node-content">
                  <el-icon class="node-icon" :style="{ color: data.color || '#409EFF' }">
                    <component :is="data.icon || 'Folder'" />
                  </el-icon>
                  <span class="node-label">{{ data.name }}</span>
                  <el-tag v-if="data.resourceCount > 0" size="small" type="info">
                    {{ data.resourceCount }}
                  </el-tag>
                  <el-tag v-if="data.status === 'disabled'" size="small" type="danger">
                    已禁用
                  </el-tag>
                </div>
    ...
    <!-- 分类详情面板 -->
    <div class="category-detail-section" v-if="selectedCategory">
      <el-card>
        <template #header>
    ...
    >
      <div class="move-content">
        <p>将分类 <strong>{{ moveCategory?.name }}</strong> 移动到：</p>
        <el-tree-select
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="confirmMove" :loading="submitting">
            确认移动
          </el-button>
        </div>
      </template>
