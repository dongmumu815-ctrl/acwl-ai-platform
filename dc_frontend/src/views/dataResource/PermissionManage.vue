<template>
  <div class="permission-manage-container">
      ...
      <el-card>
        ...
        >
          ...
          <el-table-column prop="permissionType" label="权限类型" width="100">
            <template #default="{ row }">
              <el-tag
                :type="getPermissionTypeColor(row.permissionType)"
                size="small"
              >
    ...
    >
      ...
      >
        ...
        <el-form-item label="过期时间" prop="expiresAt">
          <el-radio-group
            v-model="permissionForm.expireType"
    ...
    <!-- 访问日志对话框 -->
    <el-dialog v-model="logDialogVisible" title="访问日志" width="900px">
      <div v-if="currentPermission" class="log-content">
        <div class="log-header">
          <div class="permission-info">
            <span class="user-name">{{ currentPermission.userName }}</span>
            <el-icon><ArrowRight /></el-icon>
            <span class="resource-name">{{
              currentPermission.resourceName
            }}</span>
            <el-tag
              :type="getPermissionTypeColor(currentPermission.permissionType)"
              size="small"
            >
...
import { Coin, Document, Connection, Folder } from "@element-plus/icons-vue";
...
const activePermissions = computed(
  () => permissions.value.filter((p) => p.status === "active").length,
);
const expiredPermissions = computed(
  () => permissions.value.filter((p) => p.status === "expired").length,
);
const revokedPermissions = computed(
  () => permissions.value.filter((p) => p.status === "revoked").length,
);
 ...
 */
const batchRevoke = () => {
  const activePermissions = selectedPermissions.value.filter(
    (p) => p.status === "active",
  );
