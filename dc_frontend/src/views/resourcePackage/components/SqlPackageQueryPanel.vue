  ...
  <div class="sql-package-query-panel">
    ...
    <el-card class="query-form-card">
      ...
      >
        <el-row :gutter="20">
          <el-col
            v-for="(paramValue, paramName) in props.packageData
              ?.dynamic_params || {}"
            :key="paramName"
            :span="12"
          >
            ...
            >
              ...
              <div
                v-if="getConditionInfo(String(paramName))"
    ...
    <!-- 查询结果 -->
    <el-card v-if="queryResults" class="query-results-card">
      <template #header>
        <div class="results-header">
          <span class="card-title">查询结果</span>
          <div class="results-meta">
            <el-text type="info">
              共 {{ queryResults?.total_count || 0 }} 条记录， 耗时
              {{ queryResults?.execution_time || 0 }}ms
            </el-text>
            <!-- 新增：全字段搜索 -->
            <el-input
        ...
        >
          ...
          >
            <template #default="{ row }">
              <span v-if="typeof row[index] === 'object'">{{
        ...
        <el-dialog v-model="detailVisible" title="数据详情" width="600px">
          <el-descriptions :column="1" border>
            ...
            >
              ...
              <!-- 其他字段正常显示 -->
              <template v-else>
                <span v-if="typeof selectedRow?.[idx] === 'object'">{{
                  JSON.stringify(selectedRow?.[idx], null, 2)
                }}</span>
  ...
  if (index !== null && order) {
    filtered = [...filtered].sort((a: any[], b: any[]) => {
      ...
      if (na !== null && nb !== null) {
        ...
        cmp = String(mapTypeLabel(va ?? "")).localeCompare(
          String(mapTypeLabel(vb ?? "")),
          "zh-CN",
...
const getConditionLabel = (paramName: string) => {
  ...
  const condition = props.packageData.template_conditions.find(
    (c: any) => c.name === paramName,
  );
...
const initializeForm = () => {
  ...
  Object.keys(params).forEach((name) => {
    ...
    // 只处理未锁定的条件，若有模板条件
    const cond = props.packageData?.template_conditions?.find(
      (c: any) => c.name === name,
    );
    if (!cond || !cond.locked) {
...
const executeQuery = async () => {
  ...
  try {
    ...
    const response = await resourcePackageApi.secureQuery(
    ...
    if (queryResults.value?.error_message) {
      ...
      ElMessage.success(
        `查询成功，共找到 ${queryResults.value?.total_count || 0} 条记录`,
      );
    }
  } catch (error) {
