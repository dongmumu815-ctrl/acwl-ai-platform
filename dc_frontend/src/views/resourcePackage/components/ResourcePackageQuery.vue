<template>
  ...
  >
    <div v-loading="loading">
        ...
        <template #header>
          <div class="package-header">
            ...
            <div class="package-meta">
              <el-tag
                :type="packageData?.type === 'sql' ? 'primary' : 'success'"
              >
      ...
      <el-card
        v-if="
          packageData?.dynamic_params &&
          Object.keys(packageData.dynamic_params).length
        "
        class="query-form"
      >
        <template #header>
      ...
      <!-- 查询结果 -->
      <el-card v-if="queryResults" class="query-results">
        <template #header>
          <div class="results-header">
            <span class="section-title">查询结果</span>
            <div class="results-meta">
              <el-text type="info"
                >共 {{ queryResults.total }} 条记录，耗时
                {{ queryResults.execution_time }}ms</el-text
              >
              <el-button
                type="primary"
                size="small"
            ...
            >
              <template #default="{ row }">
                ...
                <el-tag
                  ...
                  :type="getStatusTagType(row[column.columnIndex])"
                >
    ...
    props.packageData.dynamic_conditions.forEach((condition) => {
      if (condition.required) {
        rules[condition.param_name] = [
          {
            required: true,
            message: `请输入${condition.description || condition.field}`,
            trigger: "blur",
          },
        ];
      }
...
const resultColumns = computed(() => {
  ...
  if (!queryResults.value?.columns?.length || !queryResults.value?.data?.length)
    return [];
...
const initializeForm = () => {
  ...
  queryOptions.orderDirection =
    props.packageData?.order_config?.direction || "ASC";
...
const executeQuery = async () => {
  ...
  try {
    ...
    const response = await resourcePackageApi.query(
