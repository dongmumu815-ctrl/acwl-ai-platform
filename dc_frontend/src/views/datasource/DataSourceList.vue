          ...
          <template #default="{ row }">
            <div v-if="row.last_test_at">
              ...
              <el-tag
                size="small"
                :type="
                  row.last_test_status === 'success' ? 'success' : 'danger'
                "
              >
...
const filteredDataSources = computed(() => {
  ...
  if (filterStatus.value) {
    const statusMap = {
      active: (ds: DataSource) =>
        ds.is_active && ds.last_test_status === "success",
      error: (ds: DataSource) =>
        ds.is_active && ds.last_test_status === "failed",
      disabled: (ds: DataSource) => !ds.is_active,
    };
    filtered = filtered.filter(
      statusMap[filterStatus.value as keyof typeof statusMap],
    );
  }
...
async function testConnection(dataSource: DataSource) {
  ...
  try {
    const response = await datasourceApi.testDataSourceConnection(
      dataSource.id,
    );
