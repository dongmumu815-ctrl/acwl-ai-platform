    ...
    <el-card class="mb-3" shadow="never">
      ...
      <el-form :model="filters" label-width="100px" inline>
        ...
        <el-form-item label="方法">
          <el-select v-model="filters.method" placeholder="HTTP 方法" clearable style="width: 160px">
    ...
    <el-card shadow="never">
      <el-table :data="logs" v-loading="loading" stripe height="500px">
        ...
        <el-table-column prop="status_code" label="状态码" width="120">
          <template #default="{ row }">
            <el-tag :type="statusTagType(row.status_code)" effect="plain">{{ row.status_code }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="result_status" label="结果" width="110">
          <template #default="{ row }">
            <el-tag :type="row.result_status === 'success' ? 'success' : 'danger'" effect="plain">
      ...
      <template v-if="currentDetail">
        ...
        <el-collapse class="mt-3">
          <el-collapse-item title="请求头" name="headers">
            <pre class="code-block">{{ pretty(currentDetail.request_headers) }}</pre>
          </el-collapse-item>
          <el-collapse-item title="查询参数" name="query">
            <pre class="code-block">{{ pretty(currentDetail.query_params) }}</pre>
          </el-collapse-item>
          <el-collapse-item title="请求体" name="req">
            <pre class="code-block">{{ pretty(currentDetail.request_body) }}</pre>
          </el-collapse-item>
          <el-collapse-item title="响应体" name="res">
            <pre class="code-block">{{ pretty(currentDetail.response_body) }}</pre>
          </el-collapse-item>
          <el-collapse-item title="错误信息" name="error" v-if="currentDetail.error_message">
            <pre class="code-block">{{ currentDetail.error_message }}</pre>
          </el-collapse-item>
          <el-collapse-item title="堆栈" name="stack" v-if="currentDetail.stack_trace">
...
async function loadLogs() {
  ...
  try {
    const params: any = {
      ...
      result_status: typeof filters.success === 'boolean' ? (filters.success ? 'success' : 'failure') : undefined,
    ...
    const flat = payload?.log
      ? {
          ...
          success: (() => {
            ...
            return flag !== undefined ? flag : payload?.log?.result_status === 'success'
          })()
        }
      : (payload || {})
