      ...
      >
        ...
        <el-table-column prop="need_review" label="需审读" width="90">
          <template #default="{ row }">
            <el-tag
              :type="row.need_review === 1 ? 'warning' : 'success'"
              size="small"
            >
        ...
        <el-table-column prop="sync_status" label="状态" width="110">
          <template #default="{ row }">
            <el-tag :type="statusTagType(row.sync_status)" size="small">{{
    ...
    >
      <template #header>
        <div style="display: flex; align-items: center; gap: 12px">
          ...
          <el-tag v-if="detailTableName" type="info">{{
            detailTableName
          }}</el-tag>
          <el-input
            ...
            <template v-if="col === '变更记录'" #default="{ row }">
              <el-tooltip placement="top" popper-class="pre-wrap-tooltip">
                <template #content>
                  <div
                    class="tooltip-pre"
                    v-html="formatChangeLogHtml(row[col])"
                  ></div>
                </template>
                <span class="text-pre-line">{{
                  formatChangeLogText(row[col])
                }}</span>
              </el-tooltip>
            </template>
          </el-table-column>
...
async function loadBatchDetails(batchId: string) {
  ...
  try {
    const res = await dataUploadLogsApi.getBatchDetails(batchId, {
      limit: detailLimit.value,
      offset: detailOffset.value,
      q: detailSearch.value.trim() || undefined,
    });
    const data = (res as any).data ?? res;
