<template>
  <div class="field-mapping">
    ...
    <!-- 配置详情显示 -->
    <div
      v-if="currentConfig && Object.keys(currentConfig).length > 0"
      class="config-info"
    >
      <h3>当前配置信息</h3>
      <div class="config-details">
        ...
        <p>
          <strong>配置描述：</strong>{{ currentConfig.description || "无描述" }}
        </p>
    ...
    <div class="global-settings">
      ...
      <div class="settings-content">
        ...
        <div v-if="globalSettings.mode === 'update'" class="setting-item">
          ...
          <div class="update-conditions">
            ...
            <div v-if="mappedTargetFields.length === 0" class="condition-hint">
    ...
    <div class="mapping-container">
              ...
              >
                <template #item="{ element, index }">
                  <div
                    ...
                    :class="{
                      mapped: hasSourceMapping(index),
                      custom: element.isCustom,
                  ...
                  >
                    <div class="field-content">
                      <div class="field-info">
                        ...
                        <span v-if="element.isCustom" class="custom-badge"
                          >自定义</span
                        >
                      </div>
                      ...
                      <div v-if="element.expression" class="field-expression">
                    ...
                    <div class="field-actions">
                      ...
                      <span
                        ...
                        >isCustom: {{ element.isCustom }}</span
                      >
                      <el-button
              ...
              >
                <template #item="{ element, index }">
                  <div
                    ...
                    :class="{
                      mapped: hasTargetMapping(index),
                      selected: selectedTargetIndex === index,
                    }"
                  ...
                  >
                    <div class="field-content">
                      <div class="field-info">
                        ...
                        <span v-if="!element.nullable" class="required-badge"
                          >必填</span
                        >
                      </div>
                      <div v-if="element.sample" class="field-sample">
                        示例: {{ element.sample }}
                      </div>
                      <div v-if="hasTargetMapping(index)" class="field-sample">
                        映射:
                        {{ getFieldName(getSourceFieldByTargetIndex(index)) }}
                      </div>
                    </div>
                    <div class="field-actions">
                      <el-button
      ...
      <div class="action-buttons">
        <div class="left-buttons">
          ...
          <el-button type="info" @click="previewSeaTunnelConfig"
            >预览配置</el-button
          >
        </div>
        <div class="right-buttons">
          <el-button @click="$emit('prev-step')">上一步</el-button>
          <el-button type="success" @click="saveMappingsOnly"
            >保存映射</el-button
          >
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const canProceed = computed(() => {
      return (
        positionMappings.value.length > 0 &&
      ...
      switch (sourceFieldForm.generateType) {
        ...
        case "case_when":
          return (
            ...
            sourceFieldForm.caseBranches.every(
              (branch) => branch.operator && branch.condition && branch.value,
            )
          );
        case "math":
    ...
    const mappedTargetFields = computed(() => {
      return orderedTargetFields.value
        .map((field, index) => {
    ...
    const getSourceFieldByTargetIndex = (targetIndex) => {
      const mapping = positionMappings.value.find(
        (m) => m.targetIndex === targetIndex,
      );
    ...
    const onTargetAdd = (evt) => {
      ...
      if (element && getFieldName(element)) {
        // 找到源字段的索引
        const sourceIndex = orderedSourceFields.value.findIndex(
          (f) => getFieldName(f) === getFieldName(element),
        );
        if (sourceIndex !== -1) {
          // 创建映射
    ...
    const removeMapping = (sourceIndex, targetIndex) => {
      const mappingIndex = positionMappings.value.findIndex(
        (mapping) =>
          ...
          mapping.targetIndex === targetIndex,
      );
      if (mappingIndex !== -1) {
    ...
    const removeMappingByIndex = (connectionIndex) => {
      if (
        connectionIndex >= 0 &&
        connectionIndex < positionMappings.value.length
      ) {
    ...
    const updateConnections = () => {
      ...
      console.log(
        "- 当前映射关系详情:",
        JSON.stringify(positionMappings.value),
      );

      nextTick(() => {
        if (!connectionSvg.value || !scrollArea.value) {
          console.warn(
            "FieldMapping.updateConnections: SVG容器或滚动区域不存在，无法绘制连接线",
          );
          return;
        }
        ...
        const sourceElements =
          scrollArea.value.querySelectorAll(".source-field");
        const targetElements =
          scrollArea.value.querySelectorAll(".target-field");
    ...
    const addSourceFieldImpl = (externalParams = {}) => {
      // 使用外部参数或默认值
      const defaultType =
        externalParams.defaultType ||
        (props.fieldTypeOptions && props.fieldTypeOptions[0]) ||
        "VARCHAR";
    ...
    const clearMappings = () => {
      ...
      if (deletedTargetFields.value.length > 0) {
        ...
        const sortedDeletedFields = [...deletedTargetFields.value].sort(
    ...
    const autoMapping = () => {
      ...
      // 获取未删除的源字段和目标字段
      const availableSourceFields = orderedSourceFields.value.filter(
        (field) => !field.isDeleted,
      );
      const availableTargetFields = orderedTargetFields.value.filter(
        (field) => !field.isDeleted,
      );

      // 建立1:1映射关系
      const maxMappings = Math.min(
      ...
      for (let i = 0; i < maxMappings; i++) {
        ...
        const sourceIndex = orderedSourceFields.value.findIndex(
          (field) =>
            getFieldName(field) === getFieldName(availableSourceFields[i]) &&
            !field.isDeleted,
        );

        // 找到目标字段在完整列表中的索引
        const targetIndex = orderedTargetFields.value.findIndex(
          (field) =>
            getFieldName(field) === getFieldName(availableTargetFields[i]) &&
            !field.isDeleted,
        );
      ...
      ElMessage.success(
        `已自动建立 ${positionMappings.value.length} 个字段映射`,
      );
    };
    ...
    const validateUpdateConditions = () => {
      if (globalSettings.mode === "update") {
        ...
        // 检查选择的更新条件字段是否都已配置映射
        const unmappedConditions = globalSettings.updateConditions.filter(
          (conditionField) => {
            return !mappedTargetFields.value.some(
              (field) => field.name === conditionField,
            );
          },
        );
        ...
        if (unmappedConditions.length > 0) {
          ElMessage.error(
            `以下作为更新条件的字段没有配置映射：${unmappedConditions.join(", ")}`,
          );
          return false;
        }
      }
    ...
    const generateExpression = () => {
      switch (sourceFieldForm.generateType) {
        ...
        case "case_when": {
          const branches = sourceFieldForm.caseBranches
            .map((branch) => {
              const branchCondition =
                branch.operator === "IS NULL" ||
                branch.operator === "IS NOT NULL"
    ...
    const generateSampleResult = () => {
      switch (sourceFieldForm.generateType) {
        ...
        case "concat":
          return sourceFieldForm.concatFields
            .map((field) => `[${field}]`)
            .join(sourceFieldForm.separator || "");
        case "condition":
          return `[${sourceFieldForm.trueValue}/${sourceFieldForm.falseValue}]`;
        case "case_when":
          return sourceFieldForm.caseBranches.length > 0
            ? sourceFieldForm.caseBranches[0].value
    ...
    const generateDefaultMapping = () => {
      ...
      console.log(
        "- 当前源字段详情:",
        JSON.stringify(orderedSourceFields.value),
      );
      console.log(
        "- 当前目标字段详情:",
        JSON.stringify(orderedTargetFields.value),
      );

      if (props.configId) {
        // 如果有配置ID，不生成默认映射
        console.log(
          "FieldMapping.generateDefaultMapping: 检测到configId，跳过默认映射生成",
        );
        return;
      }
      ...
      // 检查字段是否包含索引信息
      const hasSourceIndexInfo = availableSourceFields.some(
        (field) => field.sourceIndex !== undefined,
      );
      const hasTargetIndexInfo = availableTargetFields.some(
        (field) => field.targetIndex !== undefined,
      );
      ...
      if (hasSourceIndexInfo && hasTargetIndexInfo) {
        ...
        for (let i = 0; i < maxMappings; i++) {
          ...
          const sourceIndex = orderedSourceFields.value.findIndex(
            (field) =>
              getFieldName(field) === getFieldName(sourceField) &&
              !field.isDeleted,
          );

          const targetIndex = orderedTargetFields.value.findIndex(
            (field) =>
              getFieldName(field) === getFieldName(targetField) &&
              !field.isDeleted,
          );

          console.log(
            `- 映射[${i}]: 源字段=${getFieldName(sourceField)}(索引=${sourceIndex}), 目标字段=${getFieldName(targetField)}(索引=${targetIndex})`,
          );
        ...
        for (let i = 0; i < maxMappings; i++) {
          ...
          console.log(
            `- 映射[${i}]: 源字段索引=${sourceIndex}, 目标字段索引=${targetIndex}`,
          );
        ...
        if (response.data && response.data.fieldMappings) {
          ...
          if (response.data.custom_fields) {
            response.data.custom_fields.forEach((field) => {
              ...
              // 检查是否已存在同名字段
              if (
                !orderedSourceFields.value.some(
                  (existingField) => existingField.name === customField.name,
                )
              ) {
                orderedSourceFields.value.push(customField);
              }
    ...
    const loadTemplateConfig = async (templateId) => {
      try {
        console.log(
          "FieldMapping组件正在加载模板配置，templateId:",
          templateId,
        );
        ...
        if (response && response.success && response.data) {
          ...
          let fieldMappingsToUse =
            Array.isArray(templateData.field_mappings) &&
            templateData.field_mappings.length > 0
            ...
            try {
              ...
              if (mappingResponse.ok) {
                ...
                if (
                  ...
                  console.log(
                    "FieldMapping组件使用数据库回退映射（模板无映射时）:",
                    fieldMappingsToUse,
                  );
                }
              }
          ...
          if (
            orderedTargetFields.value.length === 0 &&
            props.targetFields &&
            props.targetFields.length > 0
          ) {
            console.log(
              "FieldMapping组件目标字段为空，从props重新获取:",
          ...
          console.log(
            "FieldMapping组件保留预览的源字段列表，字段数量:",
            orderedSourceFields.value.length,
          );
          console.log(
            "FieldMapping组件保留完整的目标字段列表，字段数量:",
            orderedTargetFields.value.length,
          );
          console.log("FieldMapping组件当前源字段:", orderedSourceFields.value);
          console.log(
            "FieldMapping组件当前目标字段:",
            orderedTargetFields.value,
          );
          ...
          if (fieldMappingsToUse && Array.isArray(fieldMappingsToUse)) {
            ...
            console.log(
              "🔥 loadConfigFromApi: 从field_mappings提取的自定义字段:",
              JSON.stringify(customFieldsFromMappings),
            );

            // 过滤掉已存在的同名字段，避免重复添加
            const newCustomFields = customFieldsFromMappings.filter(
              (customField) =>
                !orderedSourceFields.value.some(
                  (existingField) => existingField.name === customField.name,
                ),
            ...
            // 将不重复的自定义字段添加到源字段列表
            orderedSourceFields.value = [
              ...orderedSourceFields.value,
              ...newCustomFields,
            ];
            console.log(
              "🔥 loadConfigFromApi: 添加自定义字段后的源字段列表:",
              JSON.stringify(orderedSourceFields.value),
            );
          }
          ...
          if (
            ...
            Array.isArray(templateData.custom_fields)
          ) {
          ...
          nextTick(() => {
            ...
            if (fieldMappingsToUse && Array.isArray(fieldMappingsToUse)) {
              ...
              mappings.forEach((mappingItem) => {
                // 处理源字段 - 通过名称查找
                let sourceField = allSourceFields.find(
                  (f) => getFieldName(f) === mappingItem.sourceName,
                );

                // 如果源字段不存在且是自定义字段，从映射中恢复
                if (
                  !sourceField &&
                  (mappingItem.isCustom || mappingItem.generateType)
                ) {
                  sourceField = {
                ...
                // 处理目标字段 - 通过名称查找
                const targetField = allTargetFields.find(
                  (f) => getFieldName(f) === mappingItem.targetName,
                );

                // 如果目标字段不存在，记录警告但不影响其他字段的处理
                if (!targetField) {
                  console.warn(
                    `目标字段 '${mappingItem.targetName}' 在当前目标字段列表中不存在，跳过此映射`,
                  );
                }
                ...
                if (sourceField && targetField) {
                  ...
                  if (mappingItem.isCustom || mappingItem.generateType) {
                    ...
                    sourceField.generateType =
                      mappingItem.generateType || "constant";
                    ...
                    sourceField.conditionField =
                      mappingItem.conditionField || "";
                    sourceField.conditionValue =
                      mappingItem.conditionValue || "";
                    sourceField.conditionOperator =
                      mappingItem.conditionOperator || "=";
                    sourceField.mathExpression =
                      mappingItem.mathExpression || "";
                  ...
                  if (sourceField.isCustom) {
                    // 查找原始自定义字段以保留完整属性
                    const originalCustomField = allSourceFields.find(
                      (f) => f.isCustom && f.name === sourceField.name,
                    );
                    if (originalCustomField) {
                      ...
                      orderedSourceFieldsFromMapping.push({
                        ...
                        // 覆盖映射中提供的所有自定义属性
                        generateType:
                          mappingItem.generateType ||
                          originalCustomField.generateType,
                        expression:
                          mappingItem.expression ||
                          originalCustomField.expression,
                        dateFormat:
                          mappingItem.dateFormat ||
                          originalCustomField.dateFormat,
                        dateFunction:
                          mappingItem.dateFunction ||
                          originalCustomField.dateFunction,
                        dateInterval:
                          mappingItem.dateInterval ||
                          originalCustomField.dateInterval,
                        defaultValue:
                          mappingItem.defaultValue ||
                          originalCustomField.defaultValue,
                        constantValue:
                          mappingItem.constantValue ||
                          originalCustomField.constantValue,
                        parameterName:
                          mappingItem.parameterName ||
                          originalCustomField.parameterName,
                        separator:
                          mappingItem.separator ||
                          originalCustomField.separator,
                        trueValue:
                          mappingItem.trueValue ||
                          originalCustomField.trueValue,
                        falseValue:
                          mappingItem.falseValue ||
                          originalCustomField.falseValue,
                        caseBranches:
                          mappingItem.caseBranches ||
                          originalCustomField.caseBranches,
                        concatFields:
                          mappingItem.concatFields ||
                          originalCustomField.concatFields,
                        sample:
                          mappingItem.sample || originalCustomField.sample,
                        conditionField:
                          mappingItem.conditionField ||
                          originalCustomField.conditionField,
                        conditionValue:
                          mappingItem.conditionValue ||
                          originalCustomField.conditionValue,
                        conditionOperator:
                          mappingItem.conditionOperator ||
                          originalCustomField.conditionOperator,
                        mathExpression:
                          mappingItem.mathExpression ||
                          originalCustomField.mathExpression,
                        // 兼容旧版Babel，不使用 nullish 合并运算符
                        isDeleted:
                          mappingItem.isDeleted !== undefined
                            ? mappingItem.isDeleted
                            : originalCustomField.isDeleted,
                      });
                    } else {
                      orderedSourceFieldsFromMapping.push({
                        ...sourceField,
                  ...
                  console.warn(
                    `跳过映射：源字段 '${mappingItem.sourceName}' 或目标字段 '${mappingItem.targetName}' 不存在`,
                  );
                }
              });
              ...
              // 首先添加所有现有的目标字段
              const addedTargetNames = new Set(
                orderedTargetFieldsFromMapping.map((f) => getFieldName(f)),
              );
              ...
              const enhancedSourceFields = orderedSourceFieldsFromMapping.map(
                (field) => {
                  if (field.isCustom) {
                    // 查找原始自定义字段以保留完整属性（使用allSourceFields而不是orderedSourceFields.value）
                    const originalCustomField = allSourceFields.find(
                      (f) => f.isCustom && f.name === field.name,
                    );
                    if (originalCustomField) {
              ...
              const existingCustomFields = orderedSourceFields.value.filter(
                (f) => {
                  const isCustomAndNotInMapping =
                    f.isCustom &&
                    !enhancedSourceFields.some(
                      (enhanced) => enhanced.name === f.name,
                    );
                  const notConflictWithTarget =
                    !orderedTargetFieldsFromMapping.some(
                      (target) => target.name === f.name,
                    );
                  return isCustomAndNotInMapping && notConflictWithTarget;
                },
              );
              console.log(
                "🔥 loadConfigFromApi: 保留的现有自定义字段:",
                JSON.stringify(existingCustomFields),
              );

              // 更新字段列表和映射关系，合并现有自定义字段
              orderedSourceFields.value = [
                ...enhancedSourceFields,
                ...existingCustomFields,
              ];
              console.log(
                "🔥 loadTemplateConfig: 准备设置目标字段，orderedTargetFieldsFromMapping长度:",
                orderedTargetFieldsFromMapping.length,
              );
              console.log(
                "🔥 loadTemplateConfig: orderedTargetFieldsFromMapping内容:",
                orderedTargetFieldsFromMapping,
              );
              ...
              if (orderedTargetFieldsFromMapping.length > 0) {
                ...
                console.log(
                  "🔥 loadTemplateConfig: 使用处理后的目标字段，长度:",
                  orderedTargetFields.value.length,
                );
              } else {
                console.log(
                  "🔥 loadTemplateConfig: 处理后的目标字段为空，保留原有目标字段，长度:",
                  orderedTargetFields.value.length,
                );
              }
              ...
              console.log(
                "🔥 loadConfigFromApi: 字段映射恢复后的最终源字段列表:",
                JSON.stringify(orderedSourceFields.value),
              );
              console.log(
                "🔥 loadConfigFromApi: 自定义字段数量:",
                orderedSourceFields.value.filter((f) => f.isCustom).length,
              );

              // 加载全局设定 - 优先使用模板的global_settings；如无则尝试数据库回退响应中的global_settings
              const globalSettingsToUse =
                templateData.global_settings &&
                Object.keys(templateData.global_settings).length > 0
                  ? templateData.global_settings
                  : mappingDataForFallback &&
                      mappingDataForFallback.global_settings
                    ? mappingDataForFallback.global_settings
                    : null;
              if (globalSettingsToUse) {
                globalSettings.mode = globalSettingsToUse.mode || "insert";
                globalSettings.updateConditions =
                  globalSettingsToUse.update_conditions || [];
                console.log(
                  "FieldMapping组件加载全局设定:",
                  globalSettingsToUse,
                );
              }
              ...
              console.log(
                "🔥 loadTemplateConfig: 没有字段映射配置，保持现有字段不变",
              );
              console.log(
                "🔥 loadTemplateConfig: 当前目标字段数量:",
                orderedTargetFields.value.length,
              );
            }
    ...
    const saveMappings = async () => {
      ...
      try {
        ...
        const mappingData = positionMappings.value
          .map((mapping) => {
            ...
            if (sourceField.isCustom) {
              ...
              if (
                sourceField.generateType === "external_param" &&
                !sourceField.parameterName
              ) {
                mappingItem.parameterName = sourceField.name;
                console.log(
                  `自动设置external_param字段 '${sourceField.name}' 的parameterName为: ${sourceField.name}`,
                );
              } else {
        ...
        const customFields = orderedSourceFields.value
          ...
          .map((field) => {
            ...
            if (field.generateType === "external_param" && !parameterName) {
              ...
              console.log(
                `自动设置custom_fields中external_param字段 '${field.name}' 的parameterName为: ${field.name}`,
              );
            }
        ...
        console.log(
          "🔥 saveMappings: 保存前的源字段列表:",
          JSON.stringify(orderedSourceFields.value),
        );
        console.log(
          "🔥 saveMappings: 构建的映射数据(包含自定义字段):",
          JSON.stringify(mappingData),
        );
        console.log(
          "🔥 saveMappings: 提取的自定义字段:",
        ...
        const templateUpdateData = {
          ...
          global_settings: {
            ...
            file_type:
              store.state.sourceConfig?.file_type ||
        ...
        console.log(
          "🔥 saveMappings: 发送到后端的数据:",
          JSON.stringify(templateUpdateData),
        );
    ...
    const nextStep = async () => {
      ...
      // 构建映射数据并触发下一步事件
      const mappingData = {
        fieldMappings: positionMappings.value
          .map((mapping) => {
            const sourceField = orderedSourceFields.value[mapping.sourceIndex];
            const targetField = orderedTargetFields.value[mapping.targetIndex];
          ...
          .filter((mapping) => mapping !== null),
        customFields: orderedSourceFields.value
          .filter((field) => field.isCustom)
          .map((field) => ({
            name: field.name,
            type: field.type,
    ...
    const getFileTypeDisplay = () => {
      ...
      if (props.sourceConfig?.file_type) {
        return props.sourceConfig.file_type === "minio"
          ? "MinIO存储"
          : props.sourceConfig.file_type === "excel"
            ? "Excel文件"
            : props.sourceConfig.file_type;
      }
      ...
      if (storeConfig?.file_type) {
        return storeConfig.file_type === "minio"
          ? "MinIO存储"
          : storeConfig.file_type === "excel"
            ? "Excel文件"
      ...
      if (storeConfig?.fileConfig?.fileType) {
        return storeConfig.fileConfig.fileType === "excel"
          ? "Excel文件"
          : storeConfig.fileConfig.fileType;
      }
    ...
    watch(
      ...
      () => {
        if (globalSettings.mode === "update") {
          // 过滤掉已经不存在映射的更新条件字段
          const validConditions = globalSettings.updateConditions.filter(
            (conditionField) => {
              return mappedTargetFields.value.some(
                (field) => field.name === conditionField,
              );
    ...
    // 监听字段变化
    watch(
      [() => props.sourceFields, () => props.targetFields],
      ([newSourceFields, newTargetFields]) => {
        ...
        if (newSourceFields && newSourceFields.length > 0) {
          console.log(
            "FieldMapping组件更新源字段列表，字段数量:",
            newSourceFields.length,
          );
        ...
        if (newTargetFields && newTargetFields.length > 0) {
          console.log(
            "FieldMapping组件更新目标字段列表，字段数量:",
            newTargetFields.length,
          );
          console.log("FieldMapping组件目标字段详情:", newTargetFields);
          orderedTargetFields.value = [...newTargetFields];
          console.log(
            "FieldMapping组件更新后orderedTargetFields长度:",
            orderedTargetFields.value.length,
          );
          fieldsUpdated = true;
        } else {
          console.log(
            "FieldMapping组件未收到有效的目标字段数据:",
            newTargetFields,
          );
        }
        ...
        if (
          ...
          // 首先尝试从store中恢复字段映射
          if (
            store.state.fieldMappings &&
            Array.isArray(store.state.fieldMappings) &&
            store.state.fieldMappings.length > 0
          ) {
            console.log(
              "FieldMapping组件从store恢复字段映射（字段变化触发）:",
              store.state.fieldMappings,
            );
            positionMappings.value = store.state.fieldMappings.map(
              (mapping) => ({
                sourceIndex: mapping.sourceIndex,
                targetIndex: mapping.targetIndex,
              }),
            ...
            if (store.state.globalSettings) {
              console.log(
                "FieldMapping组件从store恢复全局设置（字段变化触发）:",
                store.state.globalSettings,
              );
              globalSettings.mode = store.state.globalSettings.mode || "insert";
              globalSettings.updateConditions =
                store.state.globalSettings.updateConditions || [];
            }
          } else {
            // 检查是否有来自模板的字段映射
            const route = getCurrentInstance()?.proxy?.$route;
            const templateId =
              route?.query?.templateId || props.templateId || props.configId;
            ...
            if (!templateId) {
              ...
              console.log(
                "FieldMapping组件检测到模板ID或配置ID，跳过默认映射生成",
              );
            }
          }
        }
    ...
    onMounted(async () => {
      ...
      const hasSourceFields =
        props.sourceFields && props.sourceFields.length > 0;
      const hasTargetFields =
        props.targetFields && props.targetFields.length > 0;
      ...
      if (storeFieldMappings) {
        // 恢复映射关系
        const mappings =
          storeFieldMappings.mappings ||
          (Array.isArray(storeFieldMappings) ? storeFieldMappings : []);
        ...
        if (customFields.length > 0) {
          ...
          const newCustomFields = restoredCustomFields.filter(
            (field) => !existingCustomFieldNames.has(field.name),
          );
    ...
    const updateFields = (
      ...
      if (newSourceFields && newSourceFields.length > 0) {
        ...
        // 检查字段是否包含索引信息
        const hasSourceIndexInfo = newSourceFields.some(
          (field) => field.sourceIndex !== undefined,
        );
        console.log("- 源字段包含索引信息:", hasSourceIndexInfo);
        ...
        if (hasSourceIndexInfo) {
          ...
          console.log(
            "- 按索引排序后的源字段:",
            JSON.stringify(sortedSourceFields),
          );
        ...
        if (
          ...
          console.log(
            "🔥 自定义字段处理后:",
            JSON.stringify(customFieldsWithFlag),
          );
          orderedSourceFields.value = [
            ...orderedSourceFields.value,
            ...customFieldsWithFlag,
          ];
          console.log(
            "🔥 最终源字段列表:",
            JSON.stringify(orderedSourceFields.value),
      ...
      if (newTargetFields && newTargetFields.length > 0) {
        ...
        // 检查字段是否包含索引信息
        const hasTargetIndexInfo = newTargetFields.some(
          (field) => field.targetIndex !== undefined,
        );
        console.log("- 目标字段包含索引信息:", hasTargetIndexInfo);
        ...
        if (hasTargetIndexInfo) {
          ...
          console.log(
            "- 按索引排序后的目标字段:",
            JSON.stringify(sortedTargetFields),
          );
      ...
      if (templateId) {
        ...
        if (
          ...
          if (
            ...
            console.log(
              "FieldMapping.updateFields: 字段包含索引信息，尝试创建映射",
            );
            ...
            for (let i = 0; i < maxMappings; i++) {
              ...
              if (sourceField && targetField) {
                console.log(
                  `- 创建映射: 源字段=${getFieldName(sourceField)}(索引=${i}), 目标字段=${getFieldName(targetField)}(索引=${i})`,
                );
                positionMappings.value.push({
                  sourceIndex: i,
                  targetIndex: i,
