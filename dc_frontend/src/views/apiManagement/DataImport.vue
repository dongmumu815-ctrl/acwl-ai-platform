<template>
  <div class="data-import">
    ...
    <el-steps :active="currentStep" align-center class="steps-container">
      ...
      <el-step
        title="字段映射"
        description="配置源字段到目标字段的映射关系"
      ></el-step>
      <el-step
        title="数据质量检查"
        description="配置数据质量检查规则"
      ></el-step>
  ...
  methods: {
    ...
    async loadTemplateConfig(templateId) {
      try {
        ...
        // 获取模板详情
        const response =
          await templateApi.default.getTemplateDetail(templateId);
        ...
        if (response.success && response.data) {
          ...
          if (templateData.config && templateData.config.minioConfig) {
            ...
            sourceConfig.file_type =
              templateData.config.fileConfig.fileType || templateData.file_type;
          } else {
            // 兼容旧的配置结构
          ...
          // 处理目标配置
          if (
            templateData.target_database &&
            Object.keys(templateData.target_database).length > 0
          ) {
            const targetConfig = {
              id: templateId,
              name: templateData.name || "模板配置",
            ...
            console.log(
              "DataImport设置后的store.state.targetConfig:",
              this.$store.state.targetConfig,
            );
          } else {
          ...
          if (
            ...
            console.log(
              "原始字段映射数据:",
              JSON.stringify(templateData.field_mappings),
            );
            console.log(
              "字段映射详情字段:",
              JSON.stringify(templateData.field_mappings_detail),
            );

            // 优先使用field_mappings_detail，如果存在的话
            const mappingsToUse =
              templateData.field_mappings_detail &&
              templateData.field_mappings_detail.length > 0
                ? templateData.field_mappings_detail
                : templateData.field_mappings;

            console.log("使用的映射数据:", JSON.stringify(mappingsToUse));

            const sourceFields = mappingsToUse
              .map((mapping, index) => {
                // 检查所有可能的源字段名称属性
                const name = mapping.sourceName || mapping.source_field || "";
                const type =
                  mapping.sourceType || mapping.data_type || "VARCHAR";
                const sourceIndex =
                ...
                console.log(
                  `处理源字段[${index}]: name=${name}, type=${type}, sourceIndex=${sourceIndex}`,
                );
              ...
              .filter((field) => {
                ...
                if (!hasName) {
                  console.warn(
                    `发现没有名称的源字段[${field.sourceIndex}]，已过滤`,
                  );
                }
                return hasName;
              }); // 过滤掉没有名称的字段

            const targetFields = mappingsToUse
              .map((mapping, index) => {
                // 检查所有可能的目标字段名称属性
                const name = mapping.targetName || mapping.target_field || "";
                const type =
                  mapping.targetType || mapping.data_type || "VARCHAR";
                const targetIndex =
                ...
                console.log(
                  `处理目标字段[${index}]: name=${name}, type=${type}, targetIndex=${targetIndex}`,
                );
              ...
              .filter((field) => {
                ...
                if (!hasName) {
                  console.warn(
                    `发现没有名称的目标字段[${field.targetIndex}]，已过滤`,
                  );
                }
                return hasName;
              }); // 过滤掉没有名称的字段
            ...
            // 确保源字段和目标字段名称唯一
            const uniqueSourceFields = Array.from(
              new Map(
                sourceFields.map((field) => [field.name, field]),
              ).values(),
            );

            const uniqueTargetFields = Array.from(
              new Map(
            ...
            if (this.sourceConfig && uniqueSourceFields.length > 0) {
              this.sourceConfig.sourceFields = uniqueSourceFields.map(
                (field) => field.name,
              );
              // 同时更新store中的源配置
              this.$store.commit("SET_SOURCE_CONFIG", this.sourceConfig);
              console.log(
                "DataImport更新源配置中的源字段:",
                this.sourceConfig.sourceFields,
              );
            }
            ...
            if (this.targetFields.length > 0) {
              ...
              console.log(
                "DataImport组件保持从TargetConfig获取的目标字段，数量:",
            ...
            console.log(
              "DataImport组件从模板加载的源字段数量:",
              this.sourceFields.length,
            );

            // 确保字段映射组件能够接收到这些字段
            console.log(
              "准备更新FieldMapping组件，源字段:",
              JSON.stringify(this.sourceFields),
            );
            console.log(
              "准备更新FieldMapping组件，目标字段:",
              JSON.stringify(this.targetFields),
            );
            ...
            this.$nextTick(() => {
              if (this.$refs.fieldMapping) {
                ...
                if (
                  ...
                  console.log(
                    "FieldMapping组件的方法:",
                    Object.keys(this.$refs.fieldMapping),
                  );
                }
              } else {
    ...
    handleDataSourceNext(config) {
      ...
      if (this.sourceConfig && this.sourceConfig.minioConfig) {
        ...
        this.sourceConfig.file_type =
          this.sourceConfig.fileConfig.fileType || "excel";
      }

      console.log(
        "DataImport handleDataSourceNext - 更新后的sourceConfig:",
        this.sourceConfig,
      );
      this.nextStep();
    },
    ...
    handleSourceConfigChange(config) {
      ...
      console.log(
        "DataImport handleSourceConfigChange - 更新后的sourceConfig:",
        this.sourceConfig,
      );
    },
    ...
    getConfigId() {
      ...
      if (
        this.sourceConfig &&
        (this.sourceConfig.config_id || this.sourceConfig.id)
      ) {
