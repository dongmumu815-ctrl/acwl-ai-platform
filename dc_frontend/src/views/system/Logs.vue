  ...
  <div class="logs-container">
    ...
    <div class="filter-bar">
      <div class="filter-left">
        ...
        <el-select v-model="filterModule" placeholder="模块" style="width: 150px" clearable>
          <el-option label="全部" value="" />
    ...
    <div class="realtime-control">
      ...
      />
      <span class="realtime-status" v-if="realtimeEnabled">
      ...
      <div class="log-detail" v-if="selectedLog">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="模块">
            <el-tag type="info">{{ getModuleLabel(selectedLog.module) }}</el-tag>
          </el-descriptions-item>
        ...
        <div class="log-details" v-if="selectedLog.details">
          <h4>详细信息</h4>
          <el-input
        ...
        <div class="log-stack" v-if="selectedLog.stack">
          <h4>堆栈信息</h4>
          <el-input
...
const logs = ref([
  {
    ...
    message: '用户登录成功',
    requestId: 'req_001',
    details: '用户张三通过用户名密码方式登录系统，登录IP: 192.168.1.100，浏览器: Chrome 120.0.0.0',
    stack: null
  },
  {
    id: 2,
    ...
    message: 'API调用频率过高',
    requestId: 'req_002',
    details: 'API接口 /api/data/query 在1分钟内被调用了150次，超过了限制阈值100次',
    stack: null
  },
  {
    id: 3,
  ...
  },
  {
    ...
    message: '数据查询超时',
    requestId: 'req_005',
    details: '查询语句执行时间超过30秒: SELECT * FROM large_table WHERE complex_condition',
    stack: null
  },
  {
    id: 8,
...
const startRealtimeLog = () => {
  realtimeTimer = setInterval(() => {
    ...
    const newLog = {
      ...
      module: ['user', 'data', 'permission', 'system', 'api'][Math.floor(Math.random() * 5)],
      user: Math.random() > 0.5 ? ['张三', '李四', '王五'][Math.floor(Math.random() * 3)] : null,
      ip: `192.168.1.${Math.floor(Math.random() * 255)}`,
      message: '实时日志消息 - ' + Math.random().toString(36).substring(7),
      requestId: Math.random() > 0.5 ? 'req_' + Math.random().toString(36).substring(7) : null,
