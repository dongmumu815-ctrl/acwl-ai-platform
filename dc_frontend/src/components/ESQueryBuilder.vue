  ...
  <div class="es-query-builder">
    ...
    <el-card class="query-card">
      ...
      <div class="es-datasource-selector">
        <el-row :gutter="16">
          ...
          <el-col :span="8">
            <el-form-item label="查询模板" v-if="props.dataResourceId">
              <el-select
            ...
            >
              ...
              <div class="condition-config">
                ...
                <div class="value-input">
                  <!-- 文本输入 -->
                  <el-input
                    v-if="condition.operator !== 'range' && condition.operator !== 'exists'"
                    v-model="condition.value"
                    placeholder="输入值"
                    style="width: 200px"
                    :disabled="condition._locked && (condition._lockType === 'full' || condition._lockType === 'value')"
                  />
                  
                  <!-- 范围输入 -->
                  <div v-else-if="condition.operator === 'range'" class="range-input">
                    <el-input
    ...
    <el-card v-if="queryResults" class="results-card">
      <template #header>
        <div class="results-header">
          ...
          <div class="results-info">
            <el-tag>总计: {{ queryResults.hits?.total?.value || queryResults.hits?.total || 0 }}</el-tag>
      ...
      <div class="results-content">
        <!-- 聚合结果卡片 -->
        <el-card v-if="queryResults?.aggregations && Object.keys(queryResults.aggregations).length > 0" class="aggregations-card" shadow="hover">
          <template #header>
            <div class="aggregations-header">
              <el-icon><PieChart /></el-icon>
              <span>聚合结果</span>
              <el-button size="small" @click="toggleAggregationsExpand" :type="aggregationsExpanded ? 'primary' : 'default'">
          ...
          <div class="aggregations-content">
            ...
            <div class="aggregations-summary">
              <el-tag v-for="(agg, name) in queryResults.aggregations" :key="name" type="info" class="agg-tag">
            ...
            <div v-if="aggregationsExpanded" class="aggregations-detail">
              <div v-for="(agg, name) in queryResults.aggregations" :key="name" class="aggregation-item">
                ...
                <div class="aggregation-data">
                  ...
                  <div v-if="agg.buckets" class="buckets-container">
                    <div class="bucket-cards">
                      <div v-for="bucket in agg.buckets" :key="bucket.key" class="bucket-card">
        ...
        <div class="results-controls">
          ...
          <el-button size="small" @click="exportResults" :disabled="!queryResults">
        ...
        <div class="pagination-wrapper">
          <el-pagination
            ...
            :total="queryResults.hits?.total?.value || queryResults.hits?.total || 0"
...
import { getESIndices, getESFieldMapping, executeESQuery as executeESQueryAPI, saveESQueryTemplate, updateESQueryTemplate } from '@/api/esQuery'
...
const canExecuteQuery = computed(() => {
  return esQueryConfig.datasourceId && (
...
const hasValidQuery = computed(() => {
  return queryResults.value && queryResults.value.hits?.hits && queryResults.value.hits.hits.length > 0
})
...
const documentFields = computed(() => {
  if (!queryResults.value?.hits?.hits || queryResults.value.hits.hits.length === 0) {
    return []
  }
...
const loadTemplates = async () => {
  ...
  try {
    ...
    console.log('💾 availableTemplates 已更新，数量:', availableTemplates.value.length)
    ...
    if (availableTemplates.value.length > 0) {
      ...
      const latestTemplate = availableTemplates.value.reduce((latest, current) => {
        // 优先使用 updated_at，其次 created_at，最后使用 id
        const latestTime = latest.updated_at || latest.created_at || latest.id
        const currentTime = current.updated_at || current.created_at || current.id
...
const getLockTypeLabel = (lockType) => {
  switch (lockType) {
    case 'full': return '完全锁定'
    case 'field': return '字段锁定'
    case 'operator': return '操作符锁定'
    case 'value': return '值锁定'
    default: return '锁定'
  }
}
...
const executeESQuery = async () => {
  ...
  try {
    ...
    if (queryType.value === 'visual') {
      ...
      // DSL查询：解析JSON字符串
      const parsedQuery = typeof fullQuery === 'string' ? JSON.parse(fullQuery) : fullQuery
...
const confirmSaveQuery = async () => {
  ...
  try {
    ...
    try {
      ...
      queryObject = formattedQuery.value ? JSON.parse(formattedQuery.value) : {}
    } catch (error) {
    ...
    if (currentTemplateId.value) {
      ...
      result = await updateESQueryTemplate(currentTemplateId.value, templateData)
      ...
      if (result && result.data && result.data.id) {
        ...
        console.log('保存成功，设置当前模板ID为编辑状态:', currentTemplateId.value)
      }
...
const parseAggregations = (aggs) => {
  ...
  for (const [aggName, aggConfig] of Object.entries(aggs)) {
    ...
    if (aggConfig.terms) {
      ...
      aggregation.params = {
        calendar_interval: aggConfig.date_histogram.calendar_interval || 
                          aggConfig.date_histogram.interval || '1d'
      }
    ...
    if (aggregation.type) {
      ...
      console.log(`📊 解析聚合: ${aggName} (${aggregation.type}) -> ${aggregation.field}`)
    } else {
...
const formatFieldValue = (value) => {
  ...
  if (typeof value === 'object') {
    if (Array.isArray(value)) {
      ...
      return `[${value.slice(0, 3).map(v => typeof v === 'object' ? JSON.stringify(v) : String(v)).join(', ')}...] (${value.length}项)`
    } else {
      // 对象类型，显示JSON字符串（截断）
...
const getAggregationSummary = (agg) => {
  if (agg.buckets && Array.isArray(agg.buckets)) {
    ...
    const totalDocs = agg.buckets.reduce((sum, bucket) => sum + bucket.doc_count, 0)
...
const exportResults = () => {
  ...
  try {
    ...
    link.setAttribute('download', `es_query_results_${new Date().getTime()}.csv`)
...
// 监听数据源变化
watch(() => esQueryConfig.datasourceId, () => {
  if (esQueryConfig.datasourceId) {
...
// 监听数据资源ID变化，自动加载模板
watch(() => props.dataResourceId, (newDataResourceId) => {
