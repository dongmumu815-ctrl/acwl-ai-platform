  ...
  <div class="es-query-builder">
    ...
    <div class="query-header">
      ...
      <div class="query-actions">
        ...
        <el-button size="small" @click="importQuery">
          <el-icon><Upload /></el-icon>
          导入查询
        </el-button>
        <el-button size="small" @click="exportQuery">
    ...
    <!-- 查询类型选择 -->
    <div v-if="queryConfig.index.length > 0" class="query-section">
      <h4 class="section-title">查询类型</h4>
      <el-radio-group
        v-model="queryConfig.queryType"
          ...
          >
            <div class="field-list">
              ...
              >
                ...
                <span v-if="field.analyzer" class="field-analyzer">
                  分析器: {{ field.analyzer }}
                </span>
              </div>
            </div>
    ...
    <!-- 查询条件构建 -->
    <div v-if="queryConfig.queryType !== 'custom'" class="query-section">
      <h4 class="section-title">
        查询条件
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="聚合类型">
              <el-select
                v-model="queryConfig.aggregation.type"
                style="width: 100%"
              >
          ...
          <el-col :span="8">
            <el-form-item label="聚合字段">
              <el-select
                v-model="queryConfig.aggregation.field"
                style="width: 100%"
                filterable
              >
                <el-option
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="6">
          <el-form-item label="排序字段">
            <el-select
              v-model="queryConfig.sort.field"
              style="width: 100%"
              clearable
            >
              <el-option
...
const queryConfig = reactive({
  ...
  index: [
    ...(props.initialIndices.length > 0 ? props.initialIndices : props.index),
  ],
  queryType: "match",
...
function generateDSLFromConditions(): string {
  ...
  if (queryConfig.sort.field) {
    ...
    const fieldInfo = availableFields.value.find(
      (f) => f.name === queryConfig.sort.field,
    );
    ...
    if (fieldInfo && fieldInfo.type === "text") {
      ...
      console.log(
        `🔧 为 text 字段 ${queryConfig.sort.field} 添加 .keyword 后缀: ${fieldName}`,
      );
    }
  ...
  if (queryConfig.enableAggregation && queryConfig.aggregation.field) {
    // 获取字段信息，判断是否需要添加 .keyword 后缀
    const fieldInfo = availableFields.value.find(
      (f) => f.name === queryConfig.aggregation.field,
    );
    ...
    if (fieldInfo && fieldInfo.type === "text") {
      ...
      console.log(
        `🔧 为 text 字段 ${queryConfig.aggregation.field} 添加 .keyword 后缀: ${fieldName}`,
      );
    }
...
function buildSingleCondition(condition: QueryCondition): any {
  ...
  if (fieldInfo && fieldInfo.type === "text" && queryType === "term") {
    ...
    console.log(
      `🔧 为 text 字段 ${field} 的 term 查询添加 .keyword 后缀: ${fieldName}`,
    );
  }
...
async function loadIndices() {
  try {
    ...
    const preselected = Array.isArray(queryConfig.index)
      ? queryConfig.index
      : [];
